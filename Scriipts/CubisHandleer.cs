...
public class CubisHandleer : MonoBehaviour
{
        ...
        if (transform.position != startpos)
        {
            ...
            if (isTriggered)
            {
                ...
                if (Input.GetMouseButtonUp(0))
                {
                    ...
                    if (nrOptions <= 1)
                    {
                        if (!nrNames.Contains(targetName)) EQG.SwitchingOrNot(this.name, ChosenOperation(targetName), targetName, PGIList, PGIIList, PGIIIList, PotentiationList, LiftList);
                        else
                        {
